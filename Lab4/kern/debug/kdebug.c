...
print_stackframe(void) {
	...
	uint32_t eip=read_eip();//获取当前eip的值，对应注释：(2) call read_eip() to get the value of eip. the type is (uint32_t);
    	...
    	for(i=0;i<=STACKFRAME_DEPTH-1&&ebp!=0;i++){//循环打印ebp和eip直至ebp地址为0或（即达到栈底）或长度超过STACKFRAME_DEPTH。对应注释：(3) from 0 .. STACKFRAME_DEPTH
        	cprintf("ebp:0x%08x eip:0x%08x args:",ebp,eip);//打印ebp和eip。对应注释：(3.1) printf value of ebp, eip
        	uint32_t *args=(uint32_t *)ebp+2;////接收ebp+2的地址，获得参数。对应注释：(3.2) (uint32_t)calling arguments [0..4] = the contents in address (unit32_t)ebp +2 [0..4]
        	for(j=0;j<=3;j++){
        	...
        	eip=*((uint32_t *)ebp+1);//调用函数的返回地址。对应注释：the calling funciton's return addr eip  = ss:[ebp+4]
            	ebp=*((uint32_t *)ebp);//上一个函数的栈针 。对应注释：the calling funciton's ebp = ss:[ebp]
    	}
